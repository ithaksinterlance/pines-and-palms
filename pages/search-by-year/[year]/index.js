...
export default function Car({ data, brandpost, cities, year, partspost, pos }) {
      ...
      <div className="flex xs:grid xs:grid-cols-1 sm:grid sm:grid-cols-1 2xs:grid 2xs:grid-cols-1  xs:mx-auto">
        <div className="w-3/4 2xs:w-full xs:w-full sm:w-full">
          <main className="mx-10 xs:mx-4 2xs:mx-4 sm:mx-4 md:mx-5 mt-10 border border-gray-100 shadow-sm my-10">
            <article>
              <h1 className="text-5xl lg:text-4xl md:text-3xl sm:text-2xl text-blue-400 font-bold py-4 sm:mt-5 md:mt-5 lg:mx-0 xs:text-xs xl:text-lg 2xs:text-xs px-5 text-justify">
                <i className="fas fa-car-garage py-5"></i> CHOOSE YOUR{" "}
              ...
              <div className="grid grid-cols-4 xs:ml-4 md:mx-4 sm:ml-0 xs:grid xs:grid-cols-2 sm:grid sm:grid-cols-3 md:grid md:grid-cols-3 2xs:grid 2xs:grid-cols-3 gap-1 2xs:mx-4 md:ml-11 mr-3 my-10 ">
                {data.map((post) => (
                  <div key={post.id}>
                    <Link
                      href="/search-by-year/[year]/[make]"
                      as={"/search-by-year/" + post.year + "/" + post.make}
                    >
                      <a>
          ...
          {brandpost.map((post) => (
            <div key={post.id}>
                <a>
                  <main className="border p-1 rounded-xl focus:border-blue-600 ">
                    <Image
                      alt={post.name}
...
export async function getStaticProps({ params }) {
  ...
  return {
    props: { data: uniqueObjectArray, brandpost, cities, year, partspost, pos },
  };
}
