...
export default function Car({ posts, cities, partspost, pos }) {
  ...
  return (
    <div>
      <Nav />
      <Head>
        <title>Search Auto Spare Parts by Year of Launch in UAE</title>
        ...
        <div className="w-3/4 2xs:w-full xs:w-full sm:w-full">
              ...
              <div className="grid grid-cols-4 xs:ml-4 md:mx-4 sm:ml-0 xs:grid xs:grid-cols-2 sm:grid sm:grid-cols-3 md:grid md:grid-cols-3 2xs:grid 2xs:grid-cols-3 gap-1 2xs:mx-4 md:ml-11 mr-3 my-10 ">
                {posts.map((post) => (
                  <div key={post.id}>
                    ...
                    >
                      <a>
                        <main className="text-center text-xl xs:text-xs xs:text-center font-mono text-blue-500 underline hover:text-blue-700 focus:text-blue-700 border border-gray-100">
                          <p className="text-base xs:text-center text-blue-500">
                            {post.year}
                          </p>
                        </main>
                      </a>
          ...
          <div className="grid grid-cols-2 xs:grid-cols-1 xs:grid py-5">
            <p className="text-base font-medium text-gray-500 xs:text-sm md:text-base p-5">
              ...
              experience. Now companies are trying to adapt to the current need
              of the trends Markets. They rely on consumer data for knowing the
              sale interest of the customers based on the experience through
              ...
              company into their targetted level. Previously there were cars
              running on petrol in which UAE is one of the largest producer and
              diesel. Since the beginning era of electric vehicle have started,
              many people are opting for electric vehicles in spite of its
              shortcomings because it is more affordable comapared to vehicle
              running on diesel or petrol. By this transition there is no
              difference in usage of irreversible energy.
              <p>
...
export async function getStaticProps() {
  ...
  const partsposts = await resp.json();

  const response = await fetch(`https://rozy.vercel.app/api/palms`);
  const pos = await response.json();

  return {
    props: { posts: uniqueObjectArray, cities, partsposts, pos },
  };
}
